...
export class ArbitrageService {
  ...
  public async getTradePairArbitrage({ tradePair }: getTradePairArbitrageDto) {
    const exchanges = this.ccxtService.getAllExchanges();

    const allPrices = exchanges.map(async (exchangeId: ExchangeId): Promise<ArbitrageInfo> => {
    ...
    const fulfilledPrices = (await Promise.allSettled(allPrices))
      ...
      .filter(({ price, timestamp }) => price !== undefined && timestamp !== undefined);
